...
export async function selectEditsAndActivity(
  ...
  type C2paActionItem = { action: string };
  ...
  if (manifest.assertions instanceof Map) {
    ...
    actionsAssertion = manifest.assertions.find((a: unknown) => (a as AssertionValue).label === 'c2pa.actions' || (a as AssertionValue).label === 'c2pa.actions.v2');
  } else {
  ...
  actions.forEach((a: C2paActionItem) => uniqueActionTypes.add(a.action));
